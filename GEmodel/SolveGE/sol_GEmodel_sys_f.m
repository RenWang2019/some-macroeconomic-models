function [c_1,c_2,s,w,r,K,L,Y] = sol_GEmodel_sys_f(beta,sigma,A,alpha,delta,X0);
% Exog. variables: {?,?,A,?,?}
...
fsolve(@cFOCs_f,X0,options);                    % call fsolve to solve the func csys_f
   function F = cFOCs_f(X)              % define the function F with X 
          ...
          F(2)      = c_1 + c_2/(1+r)- w;
          F(3)      = s + c_1 - w;       
          F(4)      = w - (1-alpha)*A*K^alpha*L^(-alpha);
          F(5)      = r - alpha*A*K^(alpha-1)*L^(1-alpha) + delta;   
          F(6)      = L - 1;
          F(7)      = K - s;
          F(8)      = Y - A*K^alpha*L^(1-alpha);
   end
end
